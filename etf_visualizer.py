...
class ETFVisualizer:
    ...
    def create_margin_trading_chart(self, margin_data: pd.DataFrame) -> go.Figure:
        """
        创建融资买入数据变化曲线图
        ...
        try:
            if margin_data.empty:
                self.logger.warning("融资买入数据为空，无法创建融资买入图表")
                # 返回一个更详细的提示图表
                fig = go.Figure()
                fig.add_annotation(
                    x=0.5, y=0.5,
                    xref="paper", yref="paper",
                    text="该ETF暂无融资买入数据<br>可能原因：<br>1. 该ETF不是融资融券标的<br>2. 当前日期无交易数据",
                    showarrow=False,
                    font=dict(size=14),
                    align="center"
                )
            ...
            # 添加融资买入额数据
            if '融资买入额' in margin_data.columns and len(margin_data) > 0:
                # 处理可能的空值或非数值数据
                margin_buy_value = margin_data['融资买入额'].iloc[0]
                if pd.notna(margin_buy_value) and margin_buy_value != '':
                    try:
                        margin_buy_amount = float(margin_buy_value)
                        fig.add_trace(go.Bar(
                            x=['融资买入额'],
                            y=[margin_buy_amount],
                            name='融资买入额',
            ...
            # 添加融资余额数据
            if '融资余额' in margin_data.columns and len(margin_data) > 0:
                # 处理可能的空值或非数值数据
                margin_balance_value = margin_data['融资余额'].iloc[0]
                if pd.notna(margin_balance_value) and margin_balance_value != '':
                    try:
                        margin_balance_amount = float(margin_balance_value)
                        fig.add_trace(go.Bar(
                            x=['融资余额'],
                            y=[margin_balance_amount],
                            name='融资余额',
            ...
            self.logger.error(f"创建融资买入图表失败: {e}")
            # 返回错误提示图表
            fig = go.Figure()
            fig.add_annotation(
                x=0.5, y=0.5,
