...
class ETFFetcher:
    ...
    def get_etf_info(self, etf_code: str, use_cache: bool = True) -> Dict:
        """
        获取ETF基本信息
        ...
        try:
            # 生成缓存键
            cache_key = f"etf_info_{etf_code}"

            # 尝试从缓存加载
            if use_cache:
                cached_data = load_cached_data(cache_key)
                if cached_data is not None:
                    self.logger.info(f"从缓存加载ETF {etf_code} 基本信息")
                    return cached_data
            ...
            # 保存到缓存
            if use_cache:
                save_cached_data(default_info, cache_key)
                self.logger.info(f"ETF {etf_code} 基本信息已缓存")
